...
local buffer = ''
local io_write = function(str) buffer = buffer .. str end
local io_flush = function()
  seamstress.tui.setAltScreen(false)
  io.write(buffer:gsub('\n', '\r\n') .. '\r\n\r\n')
  io.flush()
end
local pairs = pairs
local colors
...
return function(options)
  ...
  if not cliArgs and err then
    io.stderr:write(string.format('%s: %s\r\r', cli.name, err))
    io.stderr:write(cli.printer.generate_help_and_usage() .. '\r')
    os.exit(1)
  end
